...
### Attribution
These notes are a largely a compilation from the texts `Learn you a Haskell for Great Good` by Miran Lipovaca and `Get Programming with Haskell` by Will Kurt

## GHCI
| Command       | Description                           |
| ------------- | ------------------------------------- |
| :type or :t   | Gives the type of the function        |
| :info or :i   | Information about the function        |
| :kind or :k   | Information about the kind for a type |
| :q            | Quit                                  |
| :r            | Reloads the modules                   |
| :l <FileName> | Loads a module                        |
...
| [Char] or String      | Collection of characters, string, or list of characters   |
| (<Type>, <Type>, ...) | Tuple                                                     |
| [<Type>]              | List of type <Type>                                       |
| Ordering              | Type constructor for less than, greater than, or equal to |

### Type Variables
Type variables are just variables for types. Example: `f :: a -> a` means the function `f` accepts something of type `a` and returns something of the same type, `a`. In contrast `g :: a -> b` means the function `g` accepts something of type `a` and returns something of type `b`. Note that type `a` and `b` could possibly be the same. 

### Type Synonyms
Two names for the same type. Example: Type `[Char]` is the same as type `String`.

Type synonyms are created using the `type` keyword.

```
type Name = String
type OrderedPair = (Double, Double)
```
...
`Gender` is the type constructor. `Male`, `Female`, `Non-binary` are data constructors. This creates a concrete instance of that type. This can be extended to form complex types
...
### Type/Type Classes
A Type is an instance of a Type Class that uses the functions defined by the Type Class
Type classes define types that behave the same way (similarly)
Type classes constrain the type a function operates on.I
...
| mconcat  | Monoid - Takes list of strings and combines into single string.  |
